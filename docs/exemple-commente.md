# Exemple commenté

Extrait du classeur `INR_GPC_ONR Exemple.xlsx` : une organisation fictive ayant renseigné une partie des 32 cibles. Il montre le niveau de détail attendu dans les colonnes texte, qui portent l'essentiel de la valeur de l'exercice.

Données complètes de cet exemple : [`data/gpc-onr-exemple.json`](../data/gpc-onr-exemple.json).

## ONR 1 : Sobriété

### Cible 1.1 : En intégrant le cycle de vie complet des équipements et logiciels pour le mettre au service de la transition énergétique

| | |
|---|---|
| **Importance** | 3 / 3 |
| **Performance actuelle** | 1 / 4 |
| **Compétences** | 3 |
| **Priorité calculée** | **Intervention urgente** |

**Enjeux : opportunités et menaces**

Enjeux (opportunités et menaces) : Opportunité de réduire l'empreinte carbone globale de l'organisation et d'anticiper les futures réglementations environnementales. La menace est le coût initial potentiellement plus élevé d'équipements éco-conçus.

**Documentation de la performance actuelle**

Actuellement, le seul critère pris en compte lors des achats est le prix direct. Aucune analyse du cycle de vie (ACV) n'est demandée aux fournisseurs lors des appels d'offres

**Forces et faiblesses**

Faiblesse : manque de connaissances de l'équipe Achats sur les labels environnementaux informatiques. Force : volonté forte de la direction des achats d'évoluer vers des pratiques durables

**Stratégies d'action**

Former l'équipe Achats aux critères du Numérique Responsable (labels TCO, EPEAT). Intégrer une pondération environnementale de 15% dans les prochains appels d'offres de renouvellement de serveurs

### Cible 1.2 : En allongeant la durée d'usage des équipements, même au-delà de leur amortissement comptable

| | |
|---|---|
| **Importance** | 3 / 3 |
| **Performance actuelle** | 3 / 4 |
| **Compétences** | 2 |
| **Priorité calculée** | **Intervention à moyen terme** |

**Enjeux : opportunités et menaces**

Enjeux (opportunités et menaces) : Opportunité de faire des économies budgétaires significatives (CAPEX) et de réduire drastiquement la production de déchets électroniques (DEEE).

**Documentation de la performance actuelle**

La durée de renouvellement standard des ordinateurs portables est passée de 3 ans à 4 ans l'année dernière. Un contrat de maintenance est en place.

**Forces et faiblesses**

Force : capacité technique à réparer en interne. Faiblesse : pression psychologique de certains utilisateurs exigeant du matériel neuf et esthétique.

**Stratégies d'action**

Allonger la politique d'amortissement à 5 ans minimum. Créer un stock de pièces détachées pour faciliter les réparations rapides. Lancer une campagne de communication interne pour valoriser l'impact carbone évité en gardant son ordinateur plus longtemps

### Cible 1.3 : En éco-concevant les services numériques, et en choisissant des technologies qui contribuent aux Objectifs de développement durable (ODD)

| | |
|---|---|
| **Importance** | 3 / 3 |
| **Performance actuelle** | 2 / 4 |
| **Compétences** | 2 |
| **Priorité calculée** | **Intervention prioritaire** |

**Enjeux : opportunités et menaces**

Enjeux (opportunités et menaces) : L'opportunité est de réduire drastiquement l'empreinte environnementale des services dès leur création (démarche d'éco-conception) tout en améliorant l'expérience utilisateur (des sites plus légers chargent plus vite). Aligner les projets sur les ODD de l'ONU renforce également l'impact positif de l'organisation. La menace est la "dette technique" et la création d'"obésiciels" (logiciels inutilement lourds) qui forceront les utilisateurs à renouveler leur matériel plus vite.

**Documentation de la performance actuelle**

Documentation et mesures en place : Actuellement, les nouveaux services numériques ou sites web sont validés uniquement sur des critères fonctionnels, graphiques et financiers. Le "poids" informatique des pages et l'impact environnemental du code ne sont pas mesurés.

**Forces et faiblesses**

Force : Une équipe de développeurs et de chefs de produit très motivée par les enjeux RSE. Faiblesse : Manque de formation sur les référentiels d'éco-conception (comme le RGESN en France) et méconnaissance de la méthode pour lier concrètement un projet numérique aux ODD de l'ONU.

**Stratégies d'action**

1. Former les équipes (Designers, Développeurs, Chefs de projet) aux bonnes pratiques de l'éco-conception numérique.

2. Modifier les cahiers des charges (appels d'offres) : imposer des critères de performance environnementale aux prestataires qui créent les futurs sites ou applications de l'organisation (ex: limite de poids des images, optimisation des requêtes serveurs).

3. Créer une grille d'impact : pour chaque nouveau projet numérique, évaluer formellement à quel(s) Objectif(s) de Développement Durable (ODD) il contribue

### Cible 1.7 : En favorisant l'utilisation d'IA lorsque nécessaire uniquement et en privilégiant des IA frugales

| | |
|---|---|
| **Importance** | 3 / 3 |
| **Performance actuelle** | 1 / 4 |
| **Compétences** | 2 |
| **Priorité calculée** | **Intervention urgente** |

**Enjeux : opportunités et menaces**

Enjeux (opportunités et menaces) : L'opportunité est de maîtriser les coûts (d'infrastructure ou d'API) et de réduire l'empreinte carbone (très élevée) liée à l'entraînement et à l'utilisation quotidienne de l'IA. La menace principale est l'"effet de mode" : vouloir intégrer de l'IA partout, même quand un algorithme classique ou une simple automatisation suffirait, ce qui ferait exploser la consommation de ressources

**Documentation de la performance actuelle**

Actuellement, plusieurs projets d'IA générative (type ChatGPT) sont testés par les métiers, mais aucune mesure de leur impact énergétique n'est réalisée et aucune charte d'utilisation n'encadre le besoin réel

**Forces et faiblesses**

Force : Une équipe Data/IT curieuse et compétente techniquement. Faiblesse : Manque total de connaissances sur les concepts de "Green AI" (comment mesurer la consommation d'un modèle, comment l'optimiser) et manque de recul critique sur l'utilité réelle de l'IA pour certains cas d'usage

**Stratégies d'action**

1. Créer une grille d'évaluation préalable (Go/No-Go) pour chaque nouveau projet : l'IA est-elle indispensable ou une méthode informatique classique suffit-elle ?

2. Privilégier des modèles plus petits et spécialisés (Small Language Models) plutôt que de faire appel à des modèles massifs et très énergivores pour des tâches simples.

3. Sensibiliser les développeurs et les data scientists aux techniques d'éco-conception logicielle appliquées à l'IA (optimisation du code, limitation des calculs redondants)

## Ce que l'exemple montre

Les cibles évaluées ci-dessus partagent un même schéma : une importance élevée (3) confrontée à une performance faible (1 ou 2), ce qui les fait remonter en intervention urgente ou prioritaire. Les cibles laissées vides ne sont pas classées : elles apparaissent en « non complétées » dans l'onglet de synthèse.

La colonne « Stratégies d'action » est la sortie utile de l'exercice : elle transforme un diagnostic en plan de travail daté et attribuable.

## Voir aussi

- [Méthodologie](methodologie.md)
- [Référentiel des 32 cibles ONR](referentiel-onr.md)
